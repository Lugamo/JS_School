...
class MyClips extends Component {
  ...
  listClips(clip, index) {
    const { playlist, openForm, addPersistClip, removePersistClip } = this.props;
    ...
    for (let i = 0; i < clip.tags.length; i += 1) {
      tags= tags + ` #${clip.tags[i].charAt(0).toUpperCase() + clip.tags[i].slice(1)}` 
    }
  ...
  onRemove(id) {
    ...
    const index = playlist.list.map((e) => { return e.id; }).indexOf(id);
