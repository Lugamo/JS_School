...
class Form extends Component {
  ...
  onCreateClip() {
    const { form, newClip, closeForm, addToPlaylist} = this.props;
  ...
  onEditClip() {
    ...
    const { name, id, duration, tags, persist } = form;
    const clip = {
      id,
      type: 'clip',
      title: (name === '' ? 'Unnamed Clip' : name),
  ...
  render() {
    const { tagField } = this.state
    const { classes, form, deleteTag, clipDuration, closeForm } = this.props;
    const { open, name, duration, tags, formType } = form;
    return(
      <Dialog
          open={open}
          onClose={() => closeForm()}
